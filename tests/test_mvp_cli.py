...
class MVPCLITests(unittest.TestCase):
    ...
    def test_cli_reports_trusted_project_assurance_overview_without_state_writes(
        self,
    ) -> None:
        from acgps.workflow_engine import WorkflowEngine
        ...
        self.assertEqual(result["state_counts"], {"DRAFT": 2})
        self.assertEqual(
            result["progress_summary"]["status"],
            "TRUSTED_PROJECT_PROGRESS_SUMMARY",
        )
        self.assertEqual(
            result["audit_lineage_summary"]["status"],
            "TRUSTED_PROJECT_AUDIT_LINEAGE_SUMMARY",
        )
        self.assertEqual(
            result["next_action_queue"]["status"],
            "TRUSTED_PROJECT_NEXT_ACTION_QUEUE",
        )
        self.assertEqual(
            result["pending_decision_queue"]["status"],
